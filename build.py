...
# GoatCounter. Stats live at https://<GC_CODE>.goatcounter.com; blank the code and
# the tag stops being emitted, so a fork of this repo reports to nobody.
GC_CODE = "kanavbengani"
ANALYTICS = ('<script data-goatcounter="https://%s.goatcounter.com/count"\n'
             '        async src="//gc.zgo.at/count.js"></script>' % GC_CODE) if GC_CODE else ""
...
.hub h1{cursor:pointer;
  ...
  text-underline-offset:clamp(6px,0.62vw,10px);
  transition:text-decoration-color .16s}
.hub h1:hover,.hub h1:focus-visible{text-decoration-color:var(--ink)}
.name-tip{
...
body.editing .hub h1{cursor:text;text-decoration:none}
...
/* ---------- cards ---------- */
.card{position:absolute;z-index:3;width:clamp(178px,15.8vw,238px);
  background:var(--panel);border:1px solid var(--line);border-radius:9px;
  padding:clamp(8px,0.75vw,11px) clamp(9px,0.88vw,13px);
  cursor:pointer;text-align:left;color:inherit;font:inherit;
  display:grid;grid-template-columns:clamp(17px,1.5vw,22px) 1fr;
  gap:0 clamp(7px,0.7vw,10px);align-items:center;
...
.card .mark{width:clamp(17px,1.5vw,22px);height:clamp(17px,1.5vw,22px);
...
.card .nm{font-family:"Bricolage Grotesque",sans-serif;font-weight:600;
  font-size:clamp(11.5px,0.92vw,13.5px);letter-spacing:-.016em;line-height:1.2;display:block}
.card .rl{font-style:italic;font-weight:300;color:var(--muted);
  font-size:clamp(9.8px,0.78vw,11.5px);line-height:1.3;display:block;margin-top:1px}
.card .meta{grid-column:2;display:flex;align-items:center;gap:clamp(5px,0.5vw,7px);
...
.arc{position:absolute;transform-origin:50% 50%;white-space:nowrap;
  ...
  font-size:clamp(12px,1.1vw,16px);letter-spacing:.09em;text-transform:uppercase;
...
/* ---------------- analytics ----------------
   GoatCounter counts the pageview itself; track() is only for clicks. LOCAL keeps
   editing sessions out of the stats, and a blocked or missing script is a no-op,
   so nothing here can break the page. */
function track(path, title){
  if(LOCAL) return;
  var g = window.goatcounter;
  if(!g || typeof g.count !== "function") return;
  g.count({path:path, title:title, event:true});
}
...
function openDetail(id, origin, asEdit){
  ...
  if(!asEdit) track("card/"+id, "Card: "+id);
...
var ROOT=document.documentElement;
/* Saved card widths are stored at full scale; multiply by the same factor the
   CSS clamp applies so a hand-set width shrinks with everything else. */
function cardScale(){
...
function layout(){
  ...
  ORDER.forEach(function(id,i){
    ...
    if(o&&o.w) n.el.style.width=Math.round(o.w*cardScale())+"px";
  ...
  relax(false,16);
  var sol=fits(cx,cy,hub,W,H,true);        /* saved cards + saved labels */
  if(!sol) sol=fits(cx,cy,hub,W,H,false);  /* saved cards, labels re-solved */
  if(!sol){
    nodes.forEach(function(n,i){ n.x=saved[i].x; n.y=saved[i].y; });
    relax(true,14);
    sol=fits(cx,cy,hub,W,H,false);
  }
  if(!sol){
  ...
  /* Cards that fit are what makes the graph worth showing. If only a label has
     nowhere to go, hide that label rather than dropping to the list. */
  if(!sol && !crowded(cx,cy,hub)){
    sol=solveArcs(cx,cy,W,H,cardBoxes(),corners(cx,cy,hub.w,hub.h,0),false);
  }
...
function solveArcs(cx,cy,W,H,boxes,hubBox,usePins){
  ensureArcs();
  ...
  SECTOR_ORDER.forEach(function(cat){
    ...
    /* A hand-placed label is honoured only while it still fits. Its saved spot
       was chosen against a particular card arrangement; once the cards move
       (a smaller window falls back to automatic placement) that spot can land
       under a card, so fall through to the search and keep the rotation. */
    if(usePins && typeof o.fx==="number" && typeof o.fy==="number"){
      var px=o.fx*W, py=o.fy*H;
      if(clear(px,py)){
        placed.push(corners(px,py,lw,lh,rot));
        out.push({cat:cat,el:el,x:px,y:py,rot:rot});
        return;
      }
    }
    ...
    if(!found){
      allOk=false;
      out.push({cat:cat,el:el,x:0,y:0,rot:rot,hide:true});
      return;
    }
...
/* `usePins` is only true while the cards are still in their saved arrangement.
   A hand-placed label was positioned against that arrangement, so once the
   cards re-flow the pins are stale and the labels are solved from scratch. */
function fits(cx,cy,hub,W,H,usePins){
  if(crowded(cx,cy,hub)) return null;
  var sol=solveArcs(cx,cy,W,H,cardBoxes(),corners(cx,cy,hub.w,hub.h,0),usePins);
...
function redrawArcs(){
  ...
  applyArcs(solveArcs(cx,cy,W,H,cardBoxes(),
    corners(cx,cy,hubEl.offsetWidth,hubEl.offsetHeight,0),true));
}
function paintArcs(){
...
CATS.forEach(function(c){
  ...
  b.addEventListener("click",function(){
    if(active!==c[0]) track("filter/"+c[0], "Filter: "+c[1]);
    setFilter(active===c[0]?null:c[0]);
  });
...
/* one delegated listener covers the social icons, the card link tips and the
   links inside a detail modal — counted by host, never by full URL */
document.addEventListener("click",function(ev){
  var a = ev.target.closest && ev.target.closest('a[href^="http"]');
  if(!a) return;
  var host = (a.hostname||"").replace(/^www\\./,"");
  if(host && host !== location.hostname) track("out/"+host, "Outbound: "+host);
});

nameEl.addEventListener("click",function(){ if(!editing){ track("email/copy", "Email copied"); copyEmail(); } });
...
document.getElementById("resumeBtn").addEventListener("click",function(){
  track("resume/open", "Resume opened");
  if(rDlg.showModal) rDlg.showModal(); else rDlg.setAttribute("open","");
  sharpen();
});
dlBtn.addEventListener("click",function(){ track("resume/download", "Resume downloaded"); });
openBtn.addEventListener("click",function(){ track("resume/new-tab", "Resume in new tab"); });
