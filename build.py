...
function layout(){
  ...
  for(var pass=0;pass<140;pass++){
    for(var i=0;i<nodes.length;i++){
      for(var j=i+1;j<nodes.length;j++){
        ...
        if(ox>0&&oy>0){
          if(a.pinned&&b2.pinned) continue;
          var wa=a.pinned?0:(b2.pinned?1:0.5), wb=b2.pinned?0:(a.pinned?1:0.5);
    ...
    for(var m=0;m<nodes.length;m++){
      var n2=nodes[m];
      if(n2.pinned) continue;
      var hx=(hub.w+n2.w)/2+26-Math.abs(n2.x-cx), hy=(hub.h+n2.h)/2+18-Math.abs(n2.y-cy);
      if(hx>0&&hy>0){
        if(hx<hy) n2.x+=(n2.x<cx?-1:1)*hx; else n2.y+=(n2.y<cy?-1:1)*hy;
      }
      n2.x=Math.max(n2.w/2+16,Math.min(W-n2.w/2-16,n2.x));
  ...
  /* no room to lay this out cleanly -> show the list instead */
  if(crowded(cx,cy,hub)) ROOT.classList.add("list-mode");
}

/* Each sector gets its name set just outside the ring, rotated to follow the arc. */
var arcsEl=document.getElementById("arcs");
function drawArcs(cx,cy,W,H){
  arcsEl.innerHTML="";
  SECTOR_ORDER.forEach(function(cat){
    var s=SECTORS[cat]; if(!s) return;
    var mid=(s.from+s.to)/2, rad=mid*Math.PI/180;
    var ux=Math.cos(rad), uy=Math.sin(rad);
    /* sit beyond the furthest card along this direction */
    var reach=0;
    nodes.forEach(function(n){
      var d=(n.x-cx)*ux+(n.y-cy)*uy;
      if(d>reach){
        var corner=Math.abs(n.w/2*ux)+Math.abs(n.h/2*uy);
        reach=Math.max(reach,d+corner);
      }
    });
    var o=(LAYOUT.arcs||{})[cat]||{};
    var pad=(typeof o.pad==="number")?o.pad:38;
    var x=cx+ux*(reach+pad), y=cy+uy*(reach+pad);
    var rot=mid+90;                                  /* tangent to the circle */
    ...
    var pinned=(typeof o.fx==="number"&&typeof o.fy==="number");
    if(pinned){ x=o.fx*W; y=o.fy*H; }
    if(typeof o.rot==="number") rot=o.rot;
    if(!pinned){                                     /* keep auto labels on stage */
      x=Math.max(bw/2+10,Math.min(W-bw/2-10,x));
      y=Math.max(bh/2+8,Math.min(H-bh/2-8,y));
    }
    el.style.left=Math.round(x-bw/2)+"px";
    el.style.top=Math.round(y-bh/2)+"px";
...
if(LOCAL){
  window.__kb={
    ...
    paintWires:paintWires, drawArcs:drawArcs, dlg:dlg, openDetail:openDetail,
